    ...
    public class RapoarteController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.VenituriLuna = await _context.Facturi
                .Where(f => f.Programare.Masina.Client.UserId == userId && f.StatusPlata == StatusPlata.Platita &&f.DataEmitere >= lunaAceasta)
            ...
            ViewBag.LucrariTopLabels = System.Text.Json.JsonSerializer.Serialize( lucrariTop.Select(x => x.Denumire).ToList());
            ViewBag.LucrariTopDate = System.Text.Json.JsonSerializer.Serialize( lucrariTop.Select(x => x.Count).ToList());
